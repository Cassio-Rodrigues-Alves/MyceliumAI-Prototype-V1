...
/**
 * Representa uma conexão diferenciável.
 * Esta camada realiza uma multiplicação e adição elemento-a-elemento (y = x * w + b),
 * onde 'w' e 'b' são vetores com o mesmo número de features que a entrada.
 */
class Caminho extends tf.layers.Layer {
     ...
     */
    build(inputShape) {
        // A forma dos nossos pesos (kernel e bias) deve corresponder à dimensão das features da entrada.
        // Se a entrada é [null, 8], a dimensão das features é 8.
        const featureDim = inputShape[inputShape.length - 1];
        const weightShape = [featureDim];

        this.kernel = this.addWeight(
            `peso_${this.name || this.nome}`,
            weightShape,                       // Forma do peso (vetor)
            'float32',                         // Tipo de dado
            tf.initializers.randomNormal({ stddev: 0.02 }), // Inicializador (melhor que constante)
            undefined,                         // Regularizador (opcional)
            true                               // Treinável? Sim.
        );
    ...
    call(inputs) {
        return tf.tidy(() => {
            ...
            // A multiplicação e adição agora são elemento a elemento, graças ao broadcasting do TensorFlow.
